...
export function useAuth() {
  ...
  const loadMemberships = useCallback(async (userId) => {
    ...
    const { data, error: membershipError } = await supabase
      .from('firm_members')
      .select('id, firm_id, role, permissions, status, full_name, firms ( id, name, gstin, address, phone, email, logo_url, bank_details, invoice_prefix, next_invoice_number, reminder_grace_days )')
      .eq('user_id', userId)
      .eq('status', 'active')
